...
class PacmanGame(tk.Frame):
    def __init__(self, maze_name, master=None):
        ...
        self.food = []
        self.monster = []
        self.maze_frame = tk.Canvas(
            width=self.size[1] * size_of_block + 200,
            height=self.size[0] * size_of_block,
            bg="black",
        )
    ...
    def draw_maze(self):
        ...
        wall_block_image = wall_block_image.resize(
            (size_of_block, size_of_block), Image.ANTIALIAS
        )
        ...
        ghost_image = ghost_image.resize(
            (size_of_block, size_of_block), Image.ANTIALIAS
        )
        ...
        score_image_frame = self.maze_frame.create_image(
            self.size[1] * size_of_block + 20,
            (self.size[0] * size_of_block) // 2 - 50,
            anchor="nw",
            image=score_image,
        )
        ...
        for row in range(self.size[0]):
            for column in range(self.size[1]):
                if self.maze[row][column] == 1:
                    self.maze_frame.create_image(
                        column * size_of_block,
                        row * size_of_block,
                        anchor="nw",
                        image=wall_block_image,
                    )
    ...
    def draw_pacman(self):
        ...
        pacman_image = pacman_image.resize(
            (size_of_block, size_of_block), Image.ANTIALIAS
        )
        pacman_image = ImageTk.PhotoImage(pacman_image)

        self.pacman = self.maze_frame.create_image(
            self.pacman_pos[1] * size_of_block,
            self.pacman_pos[0] * size_of_block,
            anchor="nw",
            image=pacman_image,
        )
    ...
    def show_score(self):
        self.score_text = self.maze_frame.create_text(
            self.size[1] * size_of_block + 170,
            (self.size[0] * size_of_block) // 2,
            fill="white",
        ...
        if self.maze[pacman_path[index_path][0]][pacman_path[index_path][1]] == 2:
            for i in range(len(self.food)):
                if self.food[i][1] == pacman_path[index_path]:
                    ...
                    self.maze[pacman_path[index_path][0]][
                        pacman_path[index_path][1]
                    ] = 0
                    break

        # continue
        self.maze_frame.move(
            self.pacman,
            (pacman_path[index_path][1] - pacman_path[index_path - 1][1])
            * size_of_block,
            (pacman_path[index_path][0] - pacman_path[index_path - 1][0])
            * size_of_block,
        )
    ...
    def monster_move(self, pacman_path, monster_path, index_path, speed):
        ...
        for i in range(len(monster_path)):
            self.maze_frame.move(
                self.monster[i][0],
                (monster_path[i][index_path][1] - monster_path[i][index_path - 1][1])
                * size_of_block,
                (monster_path[i][index_path][0] - monster_path[i][index_path - 1][0])
                * size_of_block,
            )
